...
class ScreencastsHelperTest < ActionView::TestCase
  setup do
    @screencast_defaults = {
      ...
      source:       'railscasts',
      videourl:    'http://media.railscasts.com/assets/episodes/videos/360-facebook-authentication.mp4'
    }
  end
  ...
  test "should be invalid if missing required data" do
    ...
    [:title, :summary, :duration, :link, :published_at, :source, :videourl].each do |field_name|
  ...
  test "should only allow one screencast with the same video url" do
    screencast = Screencast.new(@screencast_defaults)
    screencast.videourl = screencasts(:fast_rails_commands).videourl
    assert !screencast.valid?
    assert screencast.errors[:videourl].include? "has already been taken"
  end
end
